...
Version 0.11 renders the immutable `BirthChart` result into PDF; it performs no
independent calculations. The report therefore cannot diverge through a second
astrology implementation. It carries input and calculation metadata, planetary
positions, rule evidence and sources, dignity states, the Mahadasha timeline,
and an explicit statement that it contains no prediction.

Automated checks reopen generated bytes with a separate PDF parser and require
all major sections. Release QA additionally renders every page to images and
checks typography, wrapping, clipping, tables, headers, footers, and numbering.
